...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (headerProfileAvatar && currentUser.profile_image_url) {
  ...
  // ==========================================================================
  // 8. PATIENT INTAKE & SCREENING MODAL SYSTEM (+ คัดกรองคิวใหม่ / บันทึกสัญญาณชีพ)
  // ==========================================================================
  const openIntakeModalBtn = document.getElementById('openIntakeModalBtn');
  const closeIntakeModalBtn = document.getElementById('closeIntakeModalBtn');
  const patientIntakeModal = document.getElementById('patientIntakeModal');
  const patientIntakeForm = document.getElementById('patientIntakeForm');

  if (openIntakeModalBtn && patientIntakeModal) {
    openIntakeModalBtn.addEventListener('click', (e) => {
      e.preventDefault();
      patientIntakeModal.classList.add('active');
      const intakeDateEl = document.getElementById('intakeBookingDate');
      if (intakeDateEl && !intakeDateEl.value) {
        const today = new Date().toISOString().split('T')[0];
        intakeDateEl.value = today;
      }
    });
  }

  if (closeIntakeModalBtn && patientIntakeModal) {
    closeIntakeModalBtn.addEventListener('click', () => {
      patientIntakeModal.classList.remove('active');
    });
  }

  if (patientIntakeModal) {
    patientIntakeModal.addEventListener('click', (e) => {
      if (e.target === patientIntakeModal) {
        patientIntakeModal.classList.remove('active');
      }
    });
  }
  ...
  if (patientIntakeForm) {
    patientIntakeForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const name = document.getElementById('intakePatientName').value.trim();
      const phone = document.getElementById('intakePatientPhone').value.trim();
      const bp = document.getElementById('intakeBP').value.trim();
      const pulse = document.getElementById('intakePulse').value.trim();
      const pkg = document.getElementById('intakePackageSelect').value;
      const doctor = document.getElementById('intakeDoctorSelect').value;
      const bookingDate = document.getElementById('intakeBookingDate').value;
      const bookingTime = document.getElementById('intakeBookingTime').value;

      if (!name || !phone) {
        showToast('กรุณากรอกชื่อและเบอร์โทรศัพท์ผู้ป่วยให้ครบถ้วน', 'error');
        return;
      }

      const notes = `คัดกรองโดยนักศึกษา (BP: ${bp || 'ปกติ'}, PR: ${pulse || 'ปกติ'})`;
      ...
      try {
        ...
        patientIntakeModal.classList.remove('active');
        patientIntakeForm.reset();

        showToast(`คัดกรองและออกคิวนัดหมายใหม่ให้ คุณ${name} สำเร็จ!`, 'success');

        // Dynamically insert row into queue table
        const tbody = document.querySelector('#queueTable tbody');
        if (tbody) {
          const newTr = document.createElement('tr');
          newTr.setAttribute('data-status', 'waiting');
          newTr.innerHTML = `
            <td>
              <strong style="color: var(--color-terracotta);">${bookingTime} น.</strong>
              <span style="display: block; font-size: 0.75rem; color: var(--color-muted-text);">${bookingDate}</span>
            </td>
            <td>
              <strong>คุณ${name}</strong>
              <span style="display: block; font-size: 0.78rem; color: var(--color-muted-text);">${phone}</span>
            </td>
            <td>
              <span style="font-weight: 600; color: var(--color-dark-brown);">${pkg}</span>
              <span style="display: block; font-size: 0.78rem; color: var(--color-muted-text);">${notes}</span>
            </td>
            <td>${doctor}</td>
            <td><span style="color: #7B5014; font-weight: 500;">นศ. ฝึกงาน</span></td>
            <td>
              <span class="badge-status waiting"><i class="fa-solid fa-clock"></i> รอเรียกคิว</span>
            </td>
            <td style="text-align: center;">
              <button class="btn-action-sm primary open-log-modal-btn" data-patient="คุณ${name}" data-queue="NEW" data-service="${pkg}">
                <i class="fa-solid fa-pen-to-square"></i> บันทึกชั่วโมง
              </button>
            </td>
          `;
        ...
        showToast('เกิดข้อผิดพลาดในการบันทึกคิวใหม่', 'error');
      }
    });
  }
